...
export function ImportPanel() {
  const { desktopApi, localPath, upload, sourcePreviewUrl, busy } = useAppState();
  ...
  const sourceIsImage = upload?.media_type === "image";
  const sourceIsVideo = upload?.media_type === "video";
  ...
  return (
    <section className="panel">
      ...
      {upload && sourcePreviewUrl && (
        <div className="source-preview-card">
          <strong>原素材预览</strong>
          <div className="source-preview-window">
            {sourceIsImage ? (
              <RemoteImage src={sourcePreviewUrl} alt="原图片素材" />
            ) : sourceIsVideo ? (
              <RemoteVideo src={sourcePreviewUrl} />
            ) : (
              <RemoteLink href={sourcePreviewUrl}>打开原素材</RemoteLink>
            )}
          </div>
        </div>
      )}
