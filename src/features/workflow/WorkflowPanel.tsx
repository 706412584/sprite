...
export function WorkflowPanel() {
  ...
  const suggestedStep: WorkflowStepKey = exportResult ? "export" : job ? "process" : preview || upload ? "segment" : "source";
  ...
  // 各步骤的"已完成"判定：导入图片后第 1 步立即点亮，后续步骤随产物推进逐个点亮。
  const doneSteps: Record<WorkflowStepKey, boolean> = {
    source: Boolean(upload),
    segment: Boolean(preview || job),
    matte: Boolean(preview || job),
    process: Boolean(job),
    export: Boolean(exportResult),
  };

  useEffect(() => {
    if (suggestedStep !== "source") setActiveStep(suggestedStep);
  }, [suggestedStep]);
    ...
    <div className="workflow-shell">
      ...
      <nav className="workflow-stepper" aria-label="制作步骤">
        {steps.map((step, index) => (
          <a
            key={step.key}
            className={`${step.key === activeStep ? "active" : ""} ${doneSteps[step.key] ? "done" : ""} ${step.key === suggestedStep ? "suggested" : ""}`}
            href={`#${step.key}`}
            onClick={(event) => {
      ...
      <div className="tool-stage-host workflow-stage-host">
        ...
        {activeStep === "process" && (
          <div className="workflow-column process-column" id="process">
            <section className="workflow-result-strip panel">
              <div>
                <h3>处理结果</h3>
