...
export function PreviewPanel() {
  const { settings, sampleTime, preview, busy, previewBackgroundMode, previewBackgroundColor, processPreviewZoom, processPreviewPan, sourcePreviewUrl } = useAppState();
  ...
  const sourceUrl = preview?.source_url || sourcePreviewUrl || "";
      ...
      <div className="dual-preview-grid">
        <div>
          ...
          <div className="preview-canvas">
            {sourceUrl ? (
              <div style={previewStyle}><RemoteImage src={sourceUrl} alt="原始帧预览" /></div>
            ) : (
              <span>导入素材后这里会显示原始帧。</span>
            )}
          </div>
        </div>
        <div>
